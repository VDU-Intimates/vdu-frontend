...
import SearchBar from "../common-components/search-bar"; // keep your original path

// ---- Config ----
const API_BASE =
  process.env.NEXT_PUBLIC_API_BASE?.replace(/\/$/, "") || "";

// ---- Types that match your backend /auth/me ----
type ApiUser = {
  userId: string;     // your own public id (not Mongo _id)
  fName: string;
  lName: string;
  email: string;
  address?: string | null;
  ...
  createdAt: string;
  updatedAt: string;
  // password is sent by your backend, but we won't store/use it on the client
};
...
const NavBar: React.FC = () => {
  ...
  // Load /auth/me on mount (and when token changes)
  useEffect(() => {
  ...
  const handleAccountSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        // you can swap with your toast here
  ...
  const handleDeleteAccount = () => {
    // Optional: implement a DELETE endpoint and call it here.
    // For now, just close modal (or you can remove token to simulate signout).
    setAccountOpen(false);
  };
  ...
  return (
    <nav className="flex justify-center items-center w-screen flex-wrap gap-2 overflow-visible top-navbar">
      {/* Top Navbar */}
      <div className="flex items-center max-md:justify-around justify-between min-w-screen border-b-2 px-10 h-[110px] max-md:h-[130px] max-md:p-5">
        <div className="flex items-center gap-20 max-[500px]:gap-50 w-fit h-fit max-w-screen-lg mx-4 max-md:mx-0 max-lg:w-[300px] max-xl:gap-10">
      ...
      <div className="flex items-center relative justify-start w-screen gap-4  sm:gap-6 md:gap-10 px-4 sm:px-6 md:px-10 text-l ">
        ...
        >
          ...
          <div className="flex flex-col pr-10 items-center relative shrink min-w-0 w-36
            ...
            >
              {newArrivalItems.map((item) => (
                <li
                  key={item}
                  className={`text-[11px] sm:text-sm md:text-md text-center 
          ...
          <div className="flex flex-col pr-10 items-center relative shrink-0 w-36 md:w-40
            ...
            <ul
              className={`bg-[#e0deda] cursor-pointer w-full md:w-max
